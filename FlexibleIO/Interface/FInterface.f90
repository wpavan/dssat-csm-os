...
SUBROUTINE READPEST(FILEPST, PESTID, FOUND)
    use, INTRINSIC :: iso_c_binding
    CHARACTER, dimension(*) :: FILEPST
    CHARACTER, dimension(*) :: PESTID
    INTEGER FOUND

    interface
            subroutine readPst(FILEPST, PESTID, FOUND)bind(C, name='readPest')
            import :: c_char
            CHARACTER, dimension(*) :: FILEPST
            CHARACTER, dimension(*) :: PESTID
            INTEGER :: FOUND
    ...
    call readPst(FILEPST, PESTID, FOUND)

...
void CloudOS::integrationS() {
    ...
    if (BasicS::getWeather()->getRain() >= disease->getMRRS()) {
        CloudS::removeSporesCloudByRainS(1-exp(-0.035*BasicS::getWeather()->getRain()));
    }
...
void CloudOS::addSporesCreatedS(float sporesCreated) {
